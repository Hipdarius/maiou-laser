...
function EventLog({ events, maxItems = 20 }: EventLogProps) {
        ...
        <div className="event-list">
            {displayed.map((event, i) => (
                <div key={event.id ?? i} className={`event-item ${event.type}`}>
                    <span className="event-time" title={new Date(event.timestamp).toLocaleString()}>{formatTime(event.timestamp)}</span>
